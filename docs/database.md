...
Patient-home social media accounts are **account-specific, scoped by `patient_id`** (never by device). Any device actively paired to the patient reads and manages the same set of rows: the `get_/delete_social_media_account_for_device` RPCs filter by `patient_id` only, and `device_id` is stored purely as an audit trail of the device that last wrote a row. Authenticated clients use `SupabaseSocialMediaAccountRepository`: usernames are stored in `public.social_media_accounts`, while passwords are encrypted client-side with AES-GCM before upload. The encryption key is derived **per patient** (HKDF over a fixed app pepper and the `patientId`) and cached in platform-protected `flutter_secure_storage`, so every device paired to that patient reproduces the same key and can decrypt the shared rows. Supabase RLS limits rows to linked caregivers, and `is_active_patient_device(...)` still authorizes the calling device. Unauthenticated or offline patient devices use `SecureSocialMediaAccountRepository` locally. Plaintext passwords are never sent to Supabase, logged, added to analytics, or included in sync payloads.
