...
void main() {
  group('Remember Objects Game Tests', () {
    test(
      'Difficulty settings accurately generate 3, 4, and 5 target items',
      () {
        final controllerEasy = RememberObjectsController(
        ...
        final controllerMedium = RememberObjectsController(
        ...
        final controllerHard = RememberObjectsController(
    ...
    test(
      'Phase transitions: cannot select items during memorization phase',
      () {
        ...
        controller.toggleItemSelection(firstOptionId);
        expect(
          controller.state.selectedItemIds.contains(firstOptionId),
          isTrue,
        );
    ...
    test(
      'Hints feature: increments count and highlights an unselected target item',
      () {
        final controller = RememberObjectsController();
        controller.proceedToRecall();
        ...
        expect(
          targetIds.contains(controller.state.hintHighlightedItemId),
          isTrue,
        );
      },
    );
    ...
    test(
      'Hints when all targets already selected provides supportive message',
      () {
        final controller = RememberObjectsController();
        controller.proceedToRecall();
        ...
        expect(
          ...
          contains('You have found all the items'),
        );
      },
    );

    test(
      'Scoring and Completion: returns complete GameSession with positive reinforcement',
      () {
        final controller = RememberObjectsController(
          initialDifficulty: GameDifficulty.easy,
        );
        ...
        expect(
          session.supportiveFeedbackMessage.toLowerCase(),
          isNot(contains('cognitive decline')),
        );
      },
    );
    ...
    test('Post-completion interaction is disabled', () {
      ...
      controller.toggleItemSelection(
        controller.state.selectionOptions.first.id,
      );
